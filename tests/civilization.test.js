...
test("summarizes current progress against roadmap targets", () => {
  ...
  assert.ok(summary.current.actual_lexicon_entries >= 12000);
  assert.ok(summary.current.actual_root_families >= 600);
...
test("validates expanded root inventory", () => {
  ...
  assert.equal(report.valid, true, JSON.stringify(report.errors, null, 2));
  assert.equal(report.stats.roots, 600);
  assert.equal(report.stats.lexiconEntries, 12057);
