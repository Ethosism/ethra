...
test("loads corpus and governance programs", () => {
  ...
  assert.equal(corpus.current_items, 2480);
  ...
  assert.equal(trackCounts.get("daily-dialogues"), 496);
  assert.equal(trackCounts.get("civic-law"), 372);
  assert.equal(trackCounts.get("ritual-vow"), 372);
  assert.equal(trackCounts.get("technical-software"), 496);
  assert.equal(trackCounts.get("literary-poetic"), 496);
  assert.equal(trackCounts.get("learner-graded"), 248);
...
test("recommends the next governed corpus expansion batch", () => {
  ...
  assert.equal(plan.current_items, 2480);
  assert.equal(plan.remaining_items_to_milestone, 7520);
  ...
  assert.equal(recommendations.get("daily-dialogues")?.next_item_ids[0], "daily-497");
  assert.equal(recommendations.get("civic-law")?.next_item_ids[0], "civic-373");
  assert.equal(recommendations.get("ritual-vow")?.next_item_ids[0], "ritual-373");
  assert.equal(recommendations.get("technical-software")?.next_item_ids[0], "tech-497");
  assert.equal(recommendations.get("technical-software")?.next_item_ids.at(-1), "tech-520");
  assert.equal(recommendations.get("literary-poetic")?.next_item_ids[0], "poetic-497");
  assert.equal(recommendations.get("learner-graded")?.next_item_ids[0], "learner-249");
...
test("lists and validates reviewed corpus items", () => {
  ...
  assert.equal(technicalItems.length, 496);
  ...
  assert.equal(report.stats.items, 2480);
  assert.equal(report.stats.tracks, 6);
  assert.ok(report.stats.uniqueTerms >= 1120);
});
