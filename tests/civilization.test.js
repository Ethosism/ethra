...
test("summarizes current progress against roadmap targets", () => {
  ...
  assert.ok(summary.current.actual_lexicon_entries >= 36057);
  assert.ok(summary.current.actual_root_families >= 1800);
  assert.ok(summary.current.actual_corpus_items >= 3800);
...
test("loads corpus and governance programs", () => {
  ...
  assert.equal(corpus.current_items, 3800);
  ...
  assert.equal(trackCounts.get("daily-dialogues"), 760);
  assert.equal(trackCounts.get("civic-law"), 570);
  assert.equal(trackCounts.get("ritual-vow"), 570);
  assert.equal(trackCounts.get("technical-software"), 760);
  assert.equal(trackCounts.get("literary-poetic"), 760);
  assert.equal(trackCounts.get("learner-graded"), 380);
...
test("recommends the next governed corpus expansion batch", () => {
  ...
  assert.equal(plan.current_items, 3800);
  assert.equal(plan.remaining_items_to_milestone, 6200);
  ...
  assert.equal(recommendations.get("daily-dialogues")?.next_item_ids[0], "daily-761");
  assert.equal(recommendations.get("civic-law")?.next_item_ids[0], "civic-571");
  assert.equal(recommendations.get("ritual-vow")?.next_item_ids[0], "ritual-571");
  assert.equal(recommendations.get("technical-software")?.next_item_ids[0], "tech-761");
  assert.equal(recommendations.get("technical-software")?.next_item_ids.at(-1), "tech-784");
  assert.equal(recommendations.get("literary-poetic")?.next_item_ids[0], "poetic-761");
  assert.equal(recommendations.get("learner-graded")?.next_item_ids[0], "learner-381");
  ...
  assert.ok(plan.domain_pressure.some((domain) => domain.id === "economics-provision"));
...
test("lists and validates reviewed corpus items", () => {
  ...
  assert.equal(technicalItems.length, 760);
  ...
  assert.equal(report.stats.items, 3800);
  assert.equal(report.stats.tracks, 6);
  assert.ok(report.stats.uniqueTerms >= 2039);
});
...
test("builds dictionary-grade lookup entries with corpus evidence", () => {
  ...
  assert.equal(stats.source_counts.lexicon, 36057);
  ...
  assert.equal(stats.total_entries, 36212);
  assert.equal(stats.root_families, 1800);
  assert.ok(stats.domain_counts["ai-cognition"] >= 9444);
  assert.ok(stats.domain_counts["science-math"] >= 10730);
  assert.ok(stats.domain_counts["conflict-security"] >= 1948);
  assert.ok(stats.domain_counts["education-training"] >= 5002);
  assert.ok(stats.domain_counts["emotion-psychology"] >= 3472);
  assert.ok(stats.domain_counts["nature-ecology"] >= 4490);
  assert.ok(stats.domain_counts["travel-place"] >= 16483);
  assert.ok(stats.corpus_attested_entries >= 2084);
...
test("validates expanded root inventory", () => {
  ...
  assert.equal(report.valid, true, JSON.stringify(report.errors, null, 2));
  assert.equal(report.stats.roots, 1800);
  assert.equal(report.stats.lexiconEntries, 36057);
