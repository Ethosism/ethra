...
test("summarizes current progress against roadmap targets", () => {
  ...
  assert.ok(summary.current.actual_corpus_items >= 1860);
...
test("loads corpus and governance programs", () => {
  ...
  assert.equal(corpus.current_items, 1860);
  assert.equal(corpus.remaining_items_v02, 0);
  assert.ok(corpus.tracks.some((track) => track.id === "technical-software"));
  assert.ok(corpus.tracks.every((track) => track.current_items >= 196));
  const trackCounts = new Map(corpus.tracks.map((track) => [track.id, track.current_items]));
  assert.equal(trackCounts.get("daily-dialogues"), 367);
  assert.equal(trackCounts.get("civic-law"), 282);
  assert.equal(trackCounts.get("ritual-vow"), 282);
  assert.equal(trackCounts.get("technical-software"), 366);
  assert.equal(trackCounts.get("literary-poetic"), 367);
  assert.equal(trackCounts.get("learner-graded"), 196);
...
test("recommends the next governed corpus expansion batch", () => {
  ...
  assert.equal(plan.current_items, 1860);
  assert.equal(plan.remaining_items_to_milestone, 140);
  ...
  assert.equal(recommendations.get("ritual-vow")?.recommended_items, 15);
  assert.equal(recommendations.get("technical-software")?.recommended_items, 29);
  ...
  assert.equal(recommendations.get("daily-dialogues")?.next_item_ids[0], "daily-368");
  assert.equal(recommendations.get("civic-law")?.next_item_ids[0], "civic-283");
  assert.equal(recommendations.get("ritual-vow")?.next_item_ids[0], "ritual-283");
  assert.equal(recommendations.get("technical-software")?.next_item_ids[0], "tech-367");
  assert.equal(recommendations.get("technical-software")?.next_item_ids.at(-1), "tech-395");
  assert.equal(recommendations.get("literary-poetic")?.next_item_ids[0], "poetic-368");
  assert.equal(recommendations.get("learner-graded")?.next_item_ids[0], "learner-197");
...
test("lists and validates reviewed corpus items", () => {
  ...
  assert.equal(technicalItems.length, 366);
  ...
  assert.equal(report.stats.items, 1860);
