...
test("summarizes current progress against roadmap targets", () => {
  ...
  assert.ok(summary.current.actual_lexicon_entries >= 29057);
  assert.ok(summary.current.actual_root_families >= 1450);
  assert.ok(summary.current.actual_corpus_items >= 2960);
...
test("loads corpus and governance programs", () => {
  ...
  assert.equal(corpus.current_items, 2960);
  ...
  assert.equal(trackCounts.get("daily-dialogues"), 592);
  assert.equal(trackCounts.get("civic-law"), 444);
  assert.equal(trackCounts.get("ritual-vow"), 444);
  assert.equal(trackCounts.get("technical-software"), 592);
  assert.equal(trackCounts.get("literary-poetic"), 592);
  assert.equal(trackCounts.get("learner-graded"), 296);
...
test("recommends the next governed corpus expansion batch", () => {
  ...
  assert.equal(plan.current_items, 2960);
  assert.equal(plan.remaining_items_to_milestone, 7040);
  ...
  assert.equal(recommendations.get("daily-dialogues")?.next_item_ids[0], "daily-593");
  assert.equal(recommendations.get("civic-law")?.next_item_ids[0], "civic-445");
  assert.equal(recommendations.get("ritual-vow")?.next_item_ids[0], "ritual-445");
  assert.equal(recommendations.get("technical-software")?.next_item_ids[0], "tech-593");
  assert.equal(recommendations.get("technical-software")?.next_item_ids.at(-1), "tech-616");
  assert.equal(recommendations.get("literary-poetic")?.next_item_ids[0], "poetic-593");
  assert.equal(recommendations.get("learner-graded")?.next_item_ids[0], "learner-297");
...
test("lists and validates reviewed corpus items", () => {
  ...
  assert.equal(technicalItems.length, 592);
  ...
  assert.equal(report.stats.items, 2960);
  assert.equal(report.stats.tracks, 6);
  assert.ok(report.stats.uniqueTerms >= 1454);
});
...
test("builds dictionary-grade lookup entries with corpus evidence", () => {
  ...
  assert.equal(stats.source_counts.lexicon, 29057);
  ...
  assert.equal(stats.total_entries, 29212);
  assert.equal(stats.root_families, 1450);
  assert.ok(stats.corpus_attested_entries >= 1499);
...
test("validates expanded root inventory", () => {
  ...
  assert.equal(report.valid, true, JSON.stringify(report.errors, null, 2));
  assert.equal(report.stats.roots, 1450);
  assert.equal(report.stats.lexiconEntries, 29057);
