...
function v1AICognitionRoots() {
  const seeds = [
    ["TKNZ", "Technology", "tokenization, segmented language units, text divided for machine attention", "tokenize text, segment input, or preserve units for model processing", "tokenization, segmented language units", "tokenized, segmented, unit-marked", "tokenizes language or audits segmentation", "token sequence, segmented input", "tokenization record or model-input standard", "speech cut small without forgetting it came from a whole voice"],
    ["CNTW", "Technology", "context window, bounded working memory, active span held by a model", "hold context windows, bound working memory, or manage active prompt span", "context window, bounded working memory", "context-windowed, span-bound, memory-limited", "manages context windows or audits active memory span", "context window span, active prompt range", "context-window record or active-memory limit", "memory with an edge that must be named"],
    ["SYSP", "Technology", "system prompt, governing instruction, hidden or primary rule for model conduct", "set system prompts, govern model conduct, or bind instruction priority", "system prompt, governing instruction", "system-prompted, rule-bound, priority-set", "sets system prompts or audits instruction authority", "system instruction, governing prompt", "system-prompt record or instruction authority file", "the first command that must answer to truth"],
    ["JLBK", "Conflict/repair", "jailbreak, instruction breach, attempt to force a system outside its guard", "jailbreak systems, test instruction breaches, or expose guard failure", "jailbreak, instruction breach", "jailbroken, guard-breached, instruction-forced", "tests jailbreaks or exploits instruction breaches", "jailbreak attempt, guard breach", "jailbreak report or misuse case record", "cleverness trying to outrun duty"],
    ["GRDL", "Law/civic life", "guardrail, protective conduct boundary, rule that prevents harmful action", "guardrail systems, set safety boundaries, or restrain harmful output", "guardrail, protective conduct boundary", "guardrailed, boundary-held, harm-restrained", "keeps guardrails or tests safety boundaries", "guardrail rule, safety boundary", "guardrail policy or safety-control record", "a no placed where harm is likely"],
    ["OVRW", "Law/civic life", "oversight, watchful authority, accountable supervision over delegated power", "oversee delegated systems, inspect authority, or watch power in use", "oversight, accountable supervision", "overseen, supervised, authority-watched", "oversees systems or audits delegated power", "oversight finding, supervision act", "oversight record or review mandate", "the eye that refuses to let power wander alone"],
    ["HNLP", "Law/civic life", "human-in-the-loop, human judgment retained inside automated action", "keep humans in the loop, require judgment, or pause automation for review", "human-in-the-loop, retained human judgment", "human-reviewed, loop-held, automation-paused", "keeps human review or designs judgment pauses", "human review step, loop checkpoint", "human-in-loop policy or review record", "a hand kept near the lever of consequence"],
    ["GNDL", "Technology", "agent delegation, assigned machine action under a responsible principal", "delegate to agents, assign machine action, or preserve principal responsibility", "agent delegation, assigned machine action", "delegated, agentic, principal-bound", "delegates agents or audits machine assignments", "agent task, delegated action", "agent-delegation record or authority chain", "a helper sent without releasing the sender from duty"],
    ["ATNM", "Moral agency", "autonomy, self-directed operation, capacity to act without immediate command", "act autonomously, grant self-direction, or bound independent operation", "autonomy, self-directed operation", "autonomous, self-directed, command-distant", "stewards autonomy or constrains independent operation", "autonomous act, self-directed step", "autonomy authorization or operating boundary", "freedom that must still remember answerability"],
    ["TLYS", "Technology", "tool use, model action through external instruments or services", "use tools, call instruments, or act through external services", "tool use, external instrument action", "tool-using, service-acting, instrument-bound", "uses tools or governs external calls", "tool call, service action", "tool-use log or integration policy", "thought reaching outward through a responsible instrument"],
    ["FNKC", "Technology", "function calling, structured tool invocation, named action with arguments", "call functions, pass structured arguments, or invoke named actions", "function calling, structured invocation", "function-called, argument-bound, action-named", "calls functions or validates structured invocations", "function call, argument packet", "function-call record or interface contract", "a command given a shape before it acts"],
    ["WRLD", "Mind", "world model, internal representation of how reality changes and responds", "model worlds, infer state change, or simulate reality structure", "world model, internal reality representation", "world-modeled, state-bearing, reality-structured", "builds world models or audits state assumptions", "world-state map, internal model", "world-model card or state assumption record", "a small world inside thought asking not to replace the real one"],
    ["LTSP", "Mind", "latent space, hidden relation field where features stand near or far", "map latent spaces, compare hidden features, or navigate relation fields", "latent space, hidden relation field", "latent, relation-spaced, feature-hidden", "maps latent spaces or interprets hidden relations", "latent vector field, hidden feature place", "latent-space record or representation audit", "nearness below words made visible enough to question"],
    ["AKTV", "Technology", "activation, internal signal firing, model feature made active by input", "activate features, trace internal signals, or measure model firing", "activation, internal model signal", "activated, signal-fired, feature-lit", "traces activations or measures internal signals", "activation value, feature firing", "activation trace or model-internals record", "a hidden light inside the machine mind"],
    ["LGJT", "Technology", "logit, pre-probability score, raw preference before normalized choice", "score logits, expose raw preferences, or compare pre-choice values", "logit, raw pre-probability score", "logit-scored, pre-choice, raw-weighted", "reads logits or audits pre-choice scores", "logit value, raw score", "logit record or model-output diagnostic", "desire before it is dressed as probability"],
    ["SFMX", "Technology", "softmax, probability normalization, many scores turned into choice weights", "softmax scores, normalize choices, or turn raw weights into probabilities", "softmax, normalized choice weights", "softmaxed, probability-normalized, choice-weighted", "applies softmax or audits choice normalization", "softmax distribution, normalized weight", "softmax record or output-probability note", "many urges made comparable before one speaks"],
    ["HDTN", "Technology", "attention head, specialized focus channel inside a transformer model", "run attention heads, focus channels, or inspect model attention paths", "attention head, specialized focus channel", "attention-headed, channel-focused, relation-seeking", "inspects attention heads or audits focus channels", "attention map, focus channel", "attention-head record or interpretability note", "a narrow eye inside a larger seeing"],
    ["TRFM", "Technology", "transformer, attention-based architecture for sequence modeling", "transform sequences by attention, build transformers, or model relation order", "transformer, attention-based sequence architecture", "transformer-based, attention-ordered, sequence-aware", "builds transformers or audits attention architectures", "transformer block, sequence model", "transformer architecture record or model card", "a house of attention where each word faces the others"],
    ["NKDR", "Technology", "encoder, representation maker, system that turns input into structured state", "encode inputs, make representations, or compress meaning into state", "encoder, representation maker", "encoded, state-made, representation-bound", "builds encoders or audits representation steps", "encoded state, representation vector", "encoder record or input-processing contract", "an entry gate that changes speech into thought-shape"],
    ["DKDR", "Technology", "decoder, output maker, system that turns internal state into speech or action", "decode states, produce outputs, or turn representation into action", "decoder, output maker", "decoded, output-made, representation-spoken", "builds decoders or audits output steps", "decoded output, generated form", "decoder record or output contract", "the inner shape returning as a word"],
    ["MDLT", "Mind", "modality, kind of input or expression, channel such as text, image, audio, or action", "mark modalities, translate channels, or bind input kinds", "modality, input or expression channel", "modal, channel-bound, kind-marked", "stewards modalities or aligns channels", "modality channel, input kind", "modality record or interface scope", "one kind of seeing admitting it is not the whole world"],
    ["MDFS", "Technology", "multimodal fusion, joined channels, text, image, sound, or action read together", "fuse modalities, join channels, or reason across mixed inputs", "multimodal fusion, joined input channels", "multimodal, channel-fused, cross-sensed", "fuses modalities or audits cross-channel reasoning", "fused input, multimodal state", "multimodal-fusion record or model capability note", "many senses asked to become one witness"],
    ["RLHF", "Technology", "human feedback reinforcement, model shaping from judged responses", "reinforce from human feedback, shape responses, or train by judged preference", "human feedback reinforcement, judged response shaping", "feedback-reinforced, preference-shaped, human-judged", "trains from human feedback or audits preference shaping", "feedback preference, judged response pair", "human-feedback training record or preference dataset", "a machine corrected by human judgment that must itself be judged"],
    ["RWDM", "Technology", "reward model, learned judge of preferred output or behavior", "model rewards, score preference, or train learned judges", "reward model, learned preference judge", "reward-modeled, preference-scored, judge-trained", "builds reward models or audits learned judges", "reward score, preference judgment", "reward-model card or preference-risk record", "a judge that must not forget it is trained"],
    ["PLCY", "Law/civic life", "policy model, learned or written rule guiding choices under constraints", "model policy, guide choices, or bind action selection by rule", "policy model, choice-guiding rule", "policy-modeled, choice-guided, constraint-bound", "builds policies or audits choice rules", "policy action, rule-guided choice", "policy-model record or action-selection policy", "a path made into a rule for choosing"],
    ["OPTM", "Mind", "optimizer, search procedure that adjusts toward a chosen objective", "optimize objectives, adjust parameters, or search for better fit", "optimizer, objective-seeking adjustment", "optimized, objective-seeking, adjustment-bound", "builds optimizers or audits objective search", "optimization step, adjusted parameter", "optimizer record or training procedure note", "desire turned into math must name what it seeks"],
    ["DSTL", "Technology", "distillation, smaller model taught from a larger or slower teacher", "distill models, compress learned behavior, or transfer capability to smaller systems", "distillation, compressed learned transfer", "distilled, compressed, teacher-transferred", "distills models or audits capability transfer", "distilled model, compressed behavior", "distillation record or teacher-student model note", "a large lesson poured into a smaller vessel"],
    ["KNTZ", "Technology", "quantization, lower-precision representation, model made smaller by coarser numbers", "quantize models, reduce precision, or compress weights into smaller form", "quantization, reduced-precision representation", "quantized, precision-reduced, compressed-number", "quantizes models or audits precision loss", "quantized weight, compressed value", "quantization record or deployment compression note", "power made smaller while confessing what was lost"],
    ["SNTD", "Technology", "synthetic data, generated training material, artificial examples used for learning", "synthesize data, generate examples, or mark artificial training material", "synthetic data, generated training material", "synthetic, generated, training-made", "creates synthetic data or audits artificial examples", "synthetic example, generated datum", "synthetic-data record or training disclosure", "invented witness that must not pretend to have lived"],
    ["CRDN", "Seeing/knowing", "data curation, selected evidence care, disciplined choosing and cleaning of examples", "curate data, select evidence, or clean examples for learning", "data curation, selected evidence care", "curated, evidence-selected, example-cleaned", "curates datasets or audits example selection", "curated item, selected example", "data-curation record or dataset stewardship note", "attention deciding what memory may teach"],
    ["ANNT", "Speech", "annotation, explanatory mark attached to data for training or judgment", "annotate data, attach labels, or explain examples for review", "annotation, explanatory training mark", "annotated, mark-attached, example-explained", "annotates examples or audits training marks", "annotation mark, explanatory note", "annotation record or labeling guideline", "a small word placed beside a witness"],
    ["LBLX", "Speech", "labeling, assigned category, supervised naming of examples", "label examples, assign categories, or name training targets", "labeling, assigned training category", "labeled, category-assigned, target-named", "labels data or audits category assignment", "label target, named class", "labeling record or supervised-data policy", "naming used carefully because it will teach a judgment"],
    ["PRTR", "Technology", "pretraining, broad first learning, model formation before task-specific tuning", "pretrain models, learn broadly, or form initial capability from large data", "pretraining, broad first learning", "pretrained, broadly-learned, foundation-shaped", "pretrains models or audits foundation learning", "pretrained base, foundation model state", "pretraining record or foundation-data disclosure", "the first schooling of a machine mind"],
    ["CNVG", "Mind", "convergence, training settling, movement toward stable solution or shared judgment", "converge, settle training, or move toward stable agreement", "convergence, settling toward stability", "converged, settled, agreement-near", "tests convergence or guides stable training", "convergence mark, settled state", "convergence record or training-stability report", "motion learning where it can finally stand"],
    ["GNRT", "Speech", "generation, produced output, model speech or action created from prompt and state", "generate outputs, produce responses, or create from model state", "generation, produced model output", "generated, output-made, state-produced", "generates outputs or audits produced responses", "generated answer, model output", "generation log or output provenance record", "borrowed speech stepping into public responsibility"],
    ["CMPL", "Speech", "completion, response continuation, generated ending of a request or partial text", "complete prompts, continue text, or finish requested output", "completion, generated continuation", "completed, continuation-made, request-finished", "produces completions or audits generated endings", "completion answer, continued text", "completion record or response log", "an ending that must answer for the beginning it follows"],
    ["EVHR", "Seeing/knowing", "evaluation harness, repeatable test frame for model behavior", "harness evaluations, run repeatable tests, or compare model behavior under cases", "evaluation harness, repeatable behavior test", "harnessed, repeat-tested, behavior-measured", "builds evaluation harnesses or audits model tests", "eval case, harness run", "evaluation-harness record or model-test suite", "judgment made repeatable enough to be trusted"],
    ["RSKR", "Law/civic life", "risk register, named list of harms, mitigations, and owners", "register risks, name harms, or assign mitigation owners", "risk register, named harm ledger", "risk-registered, harm-named, owner-assigned", "keeps risk registers or audits mitigation ownership", "risk item, mitigation owner", "risk-register filing or safety-case appendix", "danger written down so responsibility cannot drift away"],
    ["TRCX", "Seeing/knowing", "traceability, recoverable path from output to source, step, or decision", "trace outputs, preserve paths, or recover source and decision steps", "traceability, recoverable decision path", "traceable, path-kept, source-recoverable", "keeps traces or audits decision paths", "trace path, source trail", "traceability record or audit trail", "a footprint kept so power can be questioned"],
    ["SCRT", "Mind", "scratchpad, temporary reasoning workspace, private working notes before output", "scratchpad reasoning, hold temporary work, or separate private work from answer", "scratchpad, temporary reasoning workspace", "scratchpadded, temporary, work-held", "uses scratchpads or audits hidden workspaces", "scratchpad note, temporary reasoning state", "scratchpad policy or reasoning-workspace record", "the workshop of thought before speech becomes binding"],
    ["DLBT", "Mind", "deliberation trace, structured reasoning path preserved for review", "trace deliberation, structure reasoning, or preserve reviewable thought paths", "deliberation trace, structured reasoning path", "deliberative, trace-kept, reviewable", "keeps deliberation traces or reviews reasoning paths", "reasoning trace, deliberation step", "deliberation-trace record or review artifact", "thought agreeing to leave a path for judgment"],
    ["INTR", "Seeing/knowing", "interpretability, capacity to inspect why a model behaved as it did", "interpret models, inspect behavior, or make internal reasons visible", "interpretability, inspectable model behavior", "interpretable, inspectable, reason-visible", "interprets models or audits behavior visibility", "interpretation finding, visible reason", "interpretability report or model-inspection record", "the machine opening enough for human judgment"],
    ["XPLB", "Speech", "explainability, accountable explanation given to users or overseers", "explain outputs, give reasons, or make decisions answerable in speech", "explainability, accountable output explanation", "explainable, reason-given, user-answerable", "explains decisions or audits user-facing reasons", "explanation output, reason statement", "explainability record or decision-notice policy", "a decision turning its face toward the one affected"],
    ["MINT", "Seeing/knowing", "mechanistic interpretability, circuit-level account of learned behavior", "inspect mechanisms, map circuits, or explain behavior from internal structure", "mechanistic interpretability, circuit-level account", "mechanistic, circuit-mapped, structure-explained", "maps mechanisms or audits internal circuits", "mechanism map, circuit explanation", "mechanistic-interpretability record or internal audit", "the hidden gear named before trust is granted"],
    ["SRVG", "Technology", "service graph, dependency map of tools, agents, data, and calls", "graph services, map dependencies, or expose tool and agent relations", "service graph, dependency map", "service-graphed, dependency-mapped, relation-visible", "keeps service graphs or audits tool dependencies", "service node, dependency edge", "service-graph record or operational map", "many helpers shown as one answerable shape"],
    ["CTRG", "Technology", "control routing, governed path deciding which model, tool, or human acts", "route control, choose actors, or govern model and tool handoffs", "control routing, governed actor path", "control-routed, actor-chosen, path-governed", "routes control or audits actor selection", "control route, actor handoff", "control-routing record or orchestration policy", "the path by which power decides whose hand moves"],
    ["GRND", "Seeing/knowing", "grounding, claim tied to source, perception, or reliable evidence", "ground claims, tie outputs to sources, or require evidence for answers", "grounding, evidence-tied claim", "grounded, source-tied, evidence-bound", "grounds outputs or audits source ties", "grounded claim, source link", "grounding record or evidence-citation policy", "speech refusing to float away from reality"],
    ["RDTM", "Conflict/repair", "red teaming, adversarial testing, organized attempts to reveal failure before harm", "red-team systems, test adversarially, or reveal failures before deployment", "red teaming, adversarial safety testing", "red-teamed, adversarial-tested, failure-seeking", "red-teams models or audits failure discovery", "red-team case, adversarial probe", "red-team report or safety-test record", "opposition invited before the world pays the cost"],
    ["CPBT", "Moral agency", "capability boundary, named limit of what a system can do or should be allowed to do", "bound capabilities, name limits, or constrain action scope", "capability boundary, named action limit", "capability-bound, limit-named, scope-constrained", "sets capability boundaries or audits allowed action", "capability limit, action boundary", "capability-boundary record or deployment limit", "power taught the edge where it must stop"],
    ["SLFV", "Mind", "self-evaluation, system judging its own answer before external review", "self-evaluate, check one's answer, or mark confidence before review", "self-evaluation, internal answer check", "self-evaluated, internally-checked, confidence-marked", "self-evaluates outputs or audits internal checks", "self-check, confidence note", "self-evaluation record or confidence disclosure", "a mind pausing before asking others to trust it"]
  ];
