...
test("summarizes current progress against roadmap targets", () => {
  ...
  assert.ok(summary.current.actual_lexicon_entries >= 22057);
  assert.ok(summary.current.actual_root_families >= 1100);
...
test("builds dictionary-grade lookup entries with corpus evidence", () => {
  ...
  assert.equal(stats.source_counts.lexicon, 22057);
  ...
  assert.equal(stats.total_entries, 22212);
  assert.equal(stats.root_families, 1100);
...
test("validates expanded root inventory", () => {
  ...
  assert.equal(report.valid, true, JSON.stringify(report.errors, null, 2));
  assert.equal(report.stats.roots, 1100);
  assert.equal(report.stats.lexiconEntries, 22057);
